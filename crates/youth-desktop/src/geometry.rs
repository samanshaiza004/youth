...
mod tests {
    ...
    // --- Gate L1: Taffy-backed twins of the tests above ------------------
    //
    // Most twins reuse the exact same assertions as their old-engine
    // counterpart unmodified: they check *relative* positions/sizes (a
    // grow child's own cross-axis fill, a Row/Grid's internal spacing, a
    // button's own hit area) that hold regardless of whether the
    // *outermost* container's absolute size came from the old engine's
    // has_grow_child propagation or the new engine's Root-fill contract --
    // in every one of these fixtures the two approaches already produce
    // the same outer size, because each fixture's Root child either
    // already had an internal grow descendant (old engine already filled
    // it) or is the trivial single-leaf-child case. Two twins genuinely
    // need new, explained expected values because they pin the *outer*
    // container's own absolute size for a Root child with **no** grow
    // anywhere in its subtree -- exactly the scenario where the new
    // "single-child Root always fills" contract deliberately replaces the
    // old "only grow propagates fill" behavior. Once L3 cuts over, these
    // twins are promoted (suffix dropped) and the old-engine originals
    // above are deleted.

    #[test]
    fn root_box_text_and_button_have_reference_geometry_taffy() {
        let layout =
            taffy_adapter::layout_taffy(&counter(true), LogicalSize::new(320.0, 180.0).unwrap())
                .unwrap();
        assert_eq!(layout.nodes[&id(1)].bounds.width, 320.0);
        assert_eq!(layout.nodes[&id(2)].bounds.x, OUTER_MARGIN);
        assert_eq!(layout.nodes[&id(3)].bounds.x, OUTER_MARGIN + BOX_PADDING);
    ...
    #[test]
    fn hit_testing_obeys_bounds_and_disabled_ancestors_taffy() {
        let enabled =
            taffy_adapter::layout_taffy(&counter(true), LogicalSize::new(320.0, 180.0).unwrap())
                .unwrap();
        let button = enabled.nodes[&id(4)].bounds;
        ...
        assert_eq!(hit_test(&enabled, LogicalPoint { x: 0.0, y: 0.0 }), None);
        let disabled =
            taffy_adapter::layout_taffy(&counter(false), LogicalSize::new(320.0, 180.0).unwrap())
                .unwrap();
        assert_eq!(
            hit_test(
                &disabled,
    ...
    #[test]
    fn editor_is_an_interactive_pointer_target_taffy() {
        let tree = Tree::from_snapshot(
            TreeSnapshot {
                revision: 0,
                root: id(1),
        ...
        .unwrap();
        let layout =
            taffy_adapter::layout_taffy(&tree, LogicalSize::new(320.0, 180.0).unwrap()).unwrap();
        let editor = layout.nodes[&id(2)].bounds;
    ...
    #[test]
    fn tiny_zero_and_invalid_viewports_are_safe_taffy() {
        assert!(
            taffy_adapter::layout_taffy(&counter(true), LogicalSize::new(0.0, 0.0).unwrap())
                .is_ok()
        );
        assert!(
            taffy_adapter::layout_taffy(&counter(true), LogicalSize::new(1.0, 1.0).unwrap())
                .is_ok()
        );
    }

    #[test]
    fn row_and_grid_geometry_is_deterministic_and_row_major_taffy() {
        let layout =
            taffy_adapter::layout_taffy(&rich_layout(), LogicalSize::new(640.0, 480.0).unwrap())
                .unwrap();
        let first_row = layout.nodes[&id(4)].bounds;
        let second_row = layout.nodes[&id(5)].bounds;
        assert_eq!(second_row.x, first_row.x + first_row.width + CHILD_GAP);
    ...
    /// The old engine's Grid used two *different* formulas: `measure_container`
    /// computed a Grid's own intrinsic size from each column's own max-content
    /// width (unequal columns for unequal content), while `place()` always
    /// divided the offered width into equal `track_width` tracks regardless
    /// of content -- an internal inconsistency, not a preserved feature.
    /// Taffy's native `1fr`-per-column template is used for both intrinsic
    /// sizing and placement consistently; this test pins the new,
    /// self-consistent intrinsic behavior: an auto-sized Grid's own width is
    /// the sum of each column's own max-content contribution, not an equal
    /// division of some outer offered width (there is no outer offered
    /// width here -- the Grid is a cross-axis, non-grow child of a Column,
    /// so it sizes to its own content).
    #[test]
    fn grid_intrinsic_width_sums_each_columns_own_max_content_taffy() {
        let tree = Tree::from_snapshot(
            TreeSnapshot {
                revision: 1,
                root: id(1),
        ...
        .unwrap();
        let snapshot =
            taffy_adapter::layout_taffy(&tree, LogicalSize::new(640.0, 480.0).unwrap()).unwrap();
    ...
    #[test]
    fn single_child_root_fills_even_with_no_grow_anywhere_taffy() {
        let snapshot =
            taffy_adapter::layout_taffy(&rich_layout(), LogicalSize::new(640.0, 480.0).unwrap())
                .unwrap();
        let root = snapshot.nodes[&id(1)].bounds;
    ...
    #[test]
    fn editor_consumes_remaining_height_at_three_viewports_taffy() {
        for (width, height) in [(640.0, 480.0), (1024.0, 720.0), (1600.0, 900.0)] {
            let tree = scratchpad_shaped_tree();
            let snapshot =
                taffy_adapter::layout_taffy(&tree, LogicalSize::new(width, height).unwrap())
                    .unwrap();
            let viewport_rect = snapshot.nodes[&id(1)].bounds;
            assert_eq!(
                viewport_rect,
    ...
    #[test]
    fn unequal_weights_distribute_positive_free_space_on_top_of_intrinsic_size_taffy() {
        let tree = Tree::from_snapshot(
            TreeSnapshot {
                revision: 1,
                root: id(1),
        ...
        let width = 500.0 + OUTER_MARGIN * 2.0 + BOX_PADDING * 2.0;
        let snapshot =
            taffy_adapter::layout_taffy(&tree, LogicalSize::new(width, 200.0).unwrap()).unwrap();
        let a = snapshot.nodes[&id(3)].bounds;
        let b = snapshot.nodes[&id(4)].bounds;
        let base = 64.0 + 128.0 + CHILD_GAP;
        let free = 500.0 - base;
    ...
    #[test]
    fn free_space_not_evenly_divisible_lands_exactly_on_the_container_boundary_taffy() {
        let tree = Tree::from_snapshot(
            TreeSnapshot {
                revision: 1,
                root: id(1),
        ...
        .unwrap();
        let snapshot =
            taffy_adapter::layout_taffy(&tree, LogicalSize::new(311.0, 100.0).unwrap()).unwrap();
        let row = snapshot.nodes[&id(2)].bounds;
        let last = snapshot.nodes[&id(5)].bounds;
        let inner_right = row.x + (row.width - BOX_PADDING * 2.0) + BOX_PADDING;
        assert!(
    ...
    #[test]
    fn nested_row_inside_a_grow_aware_column_receives_its_own_forced_width_taffy() {
        let tree = Tree::from_snapshot(
            TreeSnapshot {
                revision: 1,
                root: id(1),
        ...
        .unwrap();
        let snapshot =
            taffy_adapter::layout_taffy(&tree, LogicalSize::new(400.0, 400.0).unwrap()).unwrap();
        let column = snapshot.nodes[&id(2)].bounds;
        let nested_row = snapshot.nodes[&id(4)].bounds;
    ...
    #[test]
    fn nested_column_inside_a_grow_aware_row_receives_its_own_forced_height_taffy() {
        let tree = Tree::from_snapshot(
            TreeSnapshot {
                revision: 1,
                root: id(1),
        ...
        .unwrap();
        let snapshot =
            taffy_adapter::layout_taffy(&tree, LogicalSize::new(400.0, 300.0).unwrap()).unwrap();
        let row = snapshot.nodes[&id(2)].bounds;
        let nested_column = snapshot.nodes[&id(4)].bounds;
    ...
    #[test]
    fn a_grown_editors_pointer_hit_area_extends_past_its_old_intrinsic_bounds_taffy() {
        let tree = scratchpad_shaped_tree();
        let snapshot =
            taffy_adapter::layout_taffy(&tree, LogicalSize::new(400.0, 400.0).unwrap()).unwrap();
        let editor = snapshot.nodes[&id(4)].bounds;
    ...
    /// Rewritten, not ported: the old engine's Root loop let a grow signal
    /// several levels below Root implicitly inflate an intermediate
    /// non-grow ancestor's own placed size (`has_grow_child` was checked
    /// recursively at every level). Real CSS flex-grow is per-item and
    /// does not propagate upward through an `auto`-sized ancestor this
    /// way, and a multi-child Root is deliberately just an ordinary Column
    /// under the new contract -- so the first child here sizes to its own
    /// small intrinsic content (padding plus its empty-text child's fixed
    /// GLYPH_HEIGHT), not to the full remaining viewport height.
    #[test]
    fn multi_child_root_is_an_ordinary_column_not_a_grow_propagation_taffy() {
        let tree = Tree::from_snapshot(
            TreeSnapshot {
                revision: 1,
                root: id(1),
        ...
        .unwrap();
        let snapshot =
            taffy_adapter::layout_taffy(&tree, LogicalSize::new(300.0, 300.0).unwrap()).unwrap();
        let first = snapshot.nodes[&id(2)].bounds;
        let second = snapshot.nodes[&id(4)].bounds;
        // The precise height a real flexbox engine assigns an auto-sized
        // container whose only child is a flex_grow item is a documented
        // CSS interaction (the container's own hypothetical main size,
        // when it has no definite size of its own to distribute into,
        // does not reduce to a naive "padding + child's raw intrinsic
        // size" sum the way the old hand-rolled engine's has_grow_child
        // shortcut did) -- this fixture exists only in this test, never in
        // real SDK usage, so the exact number is taken from Taffy's own
        // verified-stable output rather than hand-derived. What matters,
        // and is asserted below, is the structural contract: the first
        // child does NOT consume the full remaining viewport height the
        // way the old engine's Root propagation quirk made it do.
        assert!(
