...
| Timer | `youth-timer` (standalone repo; not yet published) | `TIMER-F` | Gate A app proof complete; probes `youth:time` scope ahead of any implementation |
...
| TIMER-F011 | Platform discovery | No representation for a durable schedule outliving one spawned app instance while Youth keeps running | Timer / Youth | Decide schedule-storage/ownership architecture (independent of `YouthAppHandle`'s worker-per-instance model) before building host-initiated delivery on top of it |
